let handler = async (m, { conn, args, participants }) => {
  ...
  let text = `
• *Mana Leaderboard Top ${len}* •
You: *${usersExp.indexOf(m.sender) + 1}* from *${usersExp.length}*

${sortedExp.slice(0, len).map(({ jid, exp }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${exp} Mana*`).join`\n`}

• *Gold Leaderboard Top ${len}* •
You: *${usersLim.indexOf(m.sender) + 1}* from *${usersLim.length}*

${sortedLim.slice(0, len).map(({ jid, limit }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${limit} Gold*`).join`\n`}
